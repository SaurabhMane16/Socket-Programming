...
Task:
UDP Client Chat App
Connects to Server (half duplex chat app with UDP )
...
int main( int argc, char *argv[])
{
    ...
    {  
        ...
        sendto(sockfd,buffer,sizeof(buffer),MSG_CONFIRM,(struct sockaddr*)&serv_addr,serv_len);
        bzero(buffer,sizeof(buffer));
        int recvbytes = recvfrom(sockfd,buffer,sizeof(buffer),MSG_WAITALL,(struct sockaddr*)&serv_addr,&serv_len);
